...
impl ScrapeField 
{
	pub fn new(span: Span, attributes: &[Attribute]) -> Self 
    {
		Self 
        {
			...
			regex: get_scrape_attr_value(symbol::REGEX, attributes),
	...
	pub fn generate_evaluation(self, field_name: String) -> Result<TokenStream2> 
    {
        ...
        else 
        {
            ...
            {
				return Err(syn::Error::new(span, "Ожидался #[token(TYPE_OF)] например regex = \"\""));
			};
			...
			if let Some(transform_fn) = self.transform_fn 
            {
				...
				Ok(quote_spanned! {span=>
					#transform_ident(wrap_values(#field_name, ::scraper_main::evaluate(#regex, doc, container).convert_from(doc))?)
				})
			} 
            else 
            {
				Ok(quote_spanned! {span=>
					wrap_values(#field_name, ::scraper_main::evaluate(#regex, doc, container).convert_from(doc))?
				})
			}
		}
	}
